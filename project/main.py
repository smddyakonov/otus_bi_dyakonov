...
from def_graphfunc import print_treemap_jbjem_ee, print_create_pie_charts, print_create_boxplot, \
    ...
    calculate_network_power, create_mapbox_scatter_point, calculate_and_merge_power_data


from def_get_xlsx import get_xlsx
from def_load_data_bd import load_data_from_sqlite
...
df_data = load_data_from_sqlite(db_path=db_path, table_name=table_name)
...
insight = html.Div([
    dbc.Row([
        html.H4('Инсайты'),
        sales_channel,
        html.H5('Инсайты использование мощности'),
        html.Div(id='table_with_insight_em'),
        html.H5('Инсайты использование электроэнергии'),
        html.Div(id='table_with_insight_ee'),
        ])
])
            ...
            dbc.Card([
                dbc.CardHeader(
                    dbc.Tabs([
                        ...
                        dbc.Tab(label='Данные', tab_id='table'),
                        dbc.Tab(label='insight', tab_id='insight'),
                    ],
                        id='card-tabs',
...
def sales_channel_filter(n_clicks, value_sales_channel, start_date, end_date, range_value):
    ...
    return  create_consumption_bar_chart(df_data=f_data, df_pnr=df_pnr, id_object = value_sales_channel)
...
def one_filter_renta(range_value):
    f_data = df_data.copy(deep=True)
    f_data = f_data[(f_data['hour'] >= range_value[0]) & (f_data['hour'] <= range_value[-1])]
    return create_histogram_with_boxplot(df_data = f_data, column='id_Объект', volt_list=list(f_data['id_Объект'].unique()))
...
def table_with_insight_em_constructor(value):
    if (len(value) == 0):
        return ''
    if value is None:
        return ''
    ...
    if isinstance(value, list):
        list_id_object.extend(value)
    else:
        list_id_object.append(value)

    f_data = df_data.copy(deep=True)
    f_ppn = df_ppn.copy(deep=True)
    f_pnr = df_pnr.copy(deep=True)
    df_ins = calculate_and_merge_power_data(df_data=f_data, df_ppn=f_ppn, df_pnr=f_pnr, id_object=value)
    df_ins = df_ins.apply(lambda x: round(x, 2) if x.dtype == 'float64' else x)

    # Преобразуем DataFrame в таблицу Dash
    element = dash_table.DataTable(
        columns=[{"name": i, "id": i} for i in df_ins.columns],
        data=df_ins.to_dict('records'),
        page_size=25
    )
...
@app.callback(
    Output(component_id='table_with_insight_ee',component_property='children'),
    Input(component_id='sales_channel',component_property='value'),
)
def table_with_insight_ee_constructor(value):
    if (len(value) == 0):
        return ''
    if value is None:
        return ''
    ...
    # Преобразуем DataFrame в таблицу Dash
    element = dash_table.DataTable(
        columns=[{"name": i, "id": i} for i in f_data_describe.columns],
        data=f_data_describe.to_dict('records'),
        #page_size=25
    )
