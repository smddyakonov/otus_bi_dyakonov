...
def create_histogram_with_boxplot(df_data, column='id_Объект', volt_list=None, nbins=73):
    """
    (df_data, column='Ур. напряж. расч.', volt_list=['ВН', 'СН1', 'СН2'], nbins=73):
...
def calculate_and_merge_power_data(df_data, df_ppn, df_pnr, id_object):
    ...
    # Вычисление сетевой мощности для второго объекта
    df_network_power = calculate_network_power_id_object(df_data, df_ppn, df_pnr, id_object=list_id_object)

    # Вычисление средней мощности для второго объекта
    df_general_power = calculate_general_power(df_data, df_pnr, id_object=list_id_object)

    # Объединение данных для второго объекта
    df_ins = pd.merge(df_network_power,
                      df_general_power,
                      how='inner',
                      left_on=['id_Объект', 'year', 'month'],
                      right_on=['id_Объект', 'год', 'месяц'])

    # Объединение с данными по сумме значений для второго объекта
    df_ins = pd.merge(df_ins,
                      df_data.groupby(['id_Объект', 'year', 'month'])[['value']].sum().reset_index(),
                      how='inner',
                      left_on=['id_Объект', 'year', 'month'],
                      right_on=['id_Объект', 'year', 'month'])

    df_ins['network/general'] = df_ins['сетевая мощность'] / df_ins['среднее потребление']
    df_ins['ЧЧИ'] = df_ins['value'] / df_ins['сетевая мощность']

    return df_ins


def calculate_network_power_id_object(df_data, df_ppn, df_pnr, id_object):
    # Преобразуем столбец с датой в формат даты (без времени)
    f_data = df_data.copy(deep=True)
    f_data['дата'] = f_data['date'].dt.date
    df_pnr['дата'] = df_pnr['RegionPeakDateHour_hour'].dt.date
    ...
    # Объединяем данные потребления с данными о плановой пиковой нагрузке по началу периода нагрузки
    f_data = f_data.merge(df_ppn[['начало_дата_час', 'конец_дата_час']], how='inner', left_on='date',
                          right_on='начало_дата_час')
    ...
    # Добавляем фильтрацию по id_object
    if isinstance(id_object, list):
        f_data = f_data[f_data['id_Объект'].isin(id_object)]
    else:
        f_data = f_data[f_data['id_Объект'] == id_object]
    ...
    # Вычисляем среднее значение максимальных нагрузок по месяцам для каждого объекта и уровня напряжения
    monthly_average = daily_max.groupby(['id_Объект', 'year', 'month', 'Ур. напряж. расч.'])[
        'value'].mean().reset_index()
    monthly_average = daily_max.groupby(['id_Объект', 'year', 'month'])['value'].sum().reset_index()
...
def calculate_general_power(df_data, df_pnr, id_object):
    list_id_object = []
    ...
    # Добавляем столбцы для года, месяца и часа, чтобы группировать данные по месяцам и часам
    df_merged['год'] = df_merged['RegionPeakDateHour_hour'].dt.year
    df_merged['месяц'] = df_merged['RegionPeakDateHour_hour'].dt.month
    df_merged['час'] = df_merged['RegionPeakDateHour_hour'].dt.hour

    # Сначала суммируем значения по объектам для каждой даты и часа
    df_summed = df_merged.groupby(['id_Объект', 'год', 'месяц', 'час'])['value'].sum().reset_index()

    # Затем вычисляем среднее значение этих сумм по месяцам и часам для указанного объекта
    result = df_summed[df_summed['id_Объект'].isin(list_id_object)].groupby(['id_Объект', 'год', 'месяц', 'час'])[
        'value'].mean().reset_index()

    # Переименовываем столбец 'value' в 'среднее потребление'
    result.rename(columns={'value': 'среднее потребление'}, inplace=True)

    # Объединяем год, месяц и час в один столбец для оси X
    result['год_месяц_час'] = result['год'].astype(str) + '-' + result['месяц'].astype(str) + '-' + result[
        'час'].astype(str)
